...
const URL_SOCKET = process.env.REACT_APP_URL_SOCKETS;
...
export function socketIoPresentations(userName, user, isInstructor) {
  return new Promise(async (resolve, reject) => {
    try {
      let response = await socketIOClient(
        `${URL_SOCKET}/presentations?name=${userName}&id=${user.ID}${isInstructor ? "&isInstructor=true" : ""
        }`
      );
      resolve(response);
    } catch (e) {
...
export async function insertAnswerSurvey(userId, surveyId, workshopId, sqlQuestions, sqlAnswers) {
  const cookies = new Cookies();
  const token = cookies.get("token");
  try {
    const response = await axios({
...
export async function DuplicateNegotiationService(user, id_negotiation, id_new_negotiation) {
  const cookies = new Cookies();
  const token = cookies.get("token");
  try {
    const response = await axios({
...
export async function updateWorkshopUser(userIdNewWorkshop, userId, newWorkshopId) {
  const cookies = new Cookies();
  const token = cookies.get("token");
  try {
    const response = await axios({
